...
public:
    ...
    void generate() {
		...
		Func xdiff("xdiff");
		xdiff(x, p) = platpos(p, 0) - (-BP_NPIX_X / 2.0f + 0.5f - cast<float>(x)) * dxdy(0);
		Func ydiff("ydiff");
		ydiff(y, p) = platpos(p, 1) - (-BP_NPIX_Y / 2.0f + 0.5f - cast<float>(y)) * dxdy(0);
		Func zdiff("zdiff");
		zdiff(p) = platpos(p, 2) - (-BP_NPIX_Y / 2.0f + 0.5f - z0(0)) * dxdy(0);
		Func R("R");
