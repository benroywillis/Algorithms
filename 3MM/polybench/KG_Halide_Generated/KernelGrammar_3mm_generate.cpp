...
public:
	...
	void generate() {
		...
		RDom rv89(0, SIZE);
		Func call99("call99");
		// no significant difference has been shown in performance between either of these two statements
		...
		// these comments are test cases meant to expose the differences in performance on might get by defining functions with different vars
		// the results are in (as of 2024-03-25): none of these statements make a difference to performance
		// the Halide docs show (and the results confirm) that vars are just names - they don't seem to expose new optimization opportunities
		// this point suggests that it's only important to know which dimensions of an expression map to the dimensions of another expression 
 		// - (think about how you would fully inline each subexpression in call113 - the vars only tell us how dimensions map to each other)
		//call113(var86, var88) += call85(var72, var74) * call99(var86, var88);
		//call113(var72, var74) += call85(var72, rv103) * call99(rv103, var74);
		call113(var101, var100) += call85(var101, rv103) * call99(rv103, var100);

		Func output("output");
		// this should refer to the vars from above
		//output(var86, var88) = call113(var86, var88);
		//output(var72, var74) = call113(var72, var74);
		output(var101, var100) = call113(var101, var100);
