...
public:
    ...
    GeneratorParam<int> direction{"direction", -1};

    // Whether the input is "real" or "complex".
    GeneratorParam<bool> input_number_type{"input_number_type", true};
    // Whether the output is "real" or "complex".
    GeneratorParam<bool> output_number_type{"output_number_type", true};
    ...
    void generate() {
        ...
        const int sign = direction;

        if (input_number_type == true) {
            if (direction == -1) {
    ...
    void schedule() {
        const int input_comps = (input_number_type == true) ? 1 : 2;
        const int output_comps = (output_number_type == true) ? 1 : 2;
